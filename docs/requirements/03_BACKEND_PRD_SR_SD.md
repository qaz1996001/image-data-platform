...
**文件 ID**: BE-PRD-SR-SD-001  
**標題**: 影像數據平台 Phase 1 — 後端（API / DB / AI Service）需求與設計  
**版本**: v2.0.0-Phase1  
**狀態**: Draft  
**建立日期**: 2025-11-24  
**最後更新**: 2025-11-25  
...
- 系統 PRD 來源：`01_SYSTEM_PRD_SR_SD.md` 中的：
  ...
  - [SYS-PRD-005 安全性與本地部署約束](01_SYSTEM_PRD_SR_SD.md#SYS-PRD-005)
- 系統 SR：SYS-SR-001 ~ SYS-SR-009 與 NFR-001 ~ 005。
...
- **對應系統需求**：SYS-PRD-005、SYS-SR-001、SYS-SR-NFR-004。
- **對應前端 PRD**：FE-PRD-001。
...
- **對應系統需求**：SYS-PRD-002、SYS-SR-002、SYS-SR-NFR-001~003。
- **對應前端 PRD**：FE-PRD-002（搜尋前準備）、匯入流程 UI。

#### BE-PRD-003 報告搜尋與詳情服務

- **描述**：提供可支援全文搜尋與多條件篩選的報告查詢 API，並在詳情端點中回傳完整報告與相關 AI 標記與專案。
- **對應系統需求**：SYS-PRD-002、SYS-SR-003、SYS-SR-004。
...
- **對應系統需求**：SYS-PRD-001、SYS-SR-005, 006, 007。
...
| BE-SR-021 | 後端必須提供 `GET /api/v1/reports/{id}`，回傳單一報告的完整資訊與關聯 AI 標記與專案摘要。 | SYS-SR-004 | `reports`, `ai_annotations`, `project_reports` |
| BE-SR-030 | 後端必須提供 `GET /api/v1/ai/annotations/{report_id}`，可依 `annotation_type` 篩選，並依 `created_at` 逆序回傳。 | SYS-SR-007 | `ai_annotations` |
...
### 3.2 UR‑003 / UR‑004 API 契約連結

- `/api/v1/studies/search`, `/api/v1/studies/{exam_id}`, `/api/v1/reports/{uid}` / `/api/v1/reports/study/{exam_id}`、`/api/v1/ai/annotations/{report_id}` 的 DTO、欄位與錯誤碼 **必須** 與 `docs/requirements/UR003_UR004_API_CONTRACT.md` 完全一致，避免 FastAPI → Django Ninja 過程再次出現欄位飄移。  
- 如需調整 `StudyListItem`, `StudyDetail`, `ReportDetailResponse`, `AIAnnotation` 等 schema，請先更新上述契約並在 PR 中明確註記，確保前端與文件能同步。
...
> 詳細 SQL / 程式碼請參考 [`docs/old/database/03_DATABASE_DESIGN.md`](../old/database/03_DATABASE_DESIGN.md) 與 [`docs/old/architecture/02_TECHNICAL_ARCHITECTURE.md`](../old/architecture/02_TECHNICAL_ARCHITECTURE.md)，此處僅整理架構層面的設計決策與其與 BE-SR 的對應。
...
- 來源：[`docs/old/database/03_DATABASE_DESIGN.md`](../old/database/03_DATABASE_DESIGN.md)
- 對應關係（摘要）：
...
- 來源：[`docs/old/architecture/02_TECHNICAL_ARCHITECTURE.md`](../old/architecture/02_TECHNICAL_ARCHITECTURE.md) 與 [`docs/old/requirements/FUNCTIONAL_SPECIFICATION.md`](../old/requirements/FUNCTIONAL_SPECIFICATION.md) 中 AI 節。
- 設計要點：
...
- 本文件 + `01_SYSTEM_PRD_SR_SD.md` + `02_FRONTEND_PRD_SR_SD.md` 共同形成：
  - **系統 / 軟體需求規格**（自 UR / NFR 整理而來的 SYS-SR / FE-SR / BE-SR）。
  - **軟體架構與詳細設計**（由 architecture / DB / API 舊文件承接，並在此文件中掛上需求 ID 與追溯關係）。
- 後續若需符合醫療器材軟體完整程序：
  - 應再補充：
...
*至此，系統層 / 前端層 / 後端層 PRD / SR / SD 的最小骨架與交叉連結已建立；接下來可以依實際實作與審查結果逐步充實細節與更新版本。