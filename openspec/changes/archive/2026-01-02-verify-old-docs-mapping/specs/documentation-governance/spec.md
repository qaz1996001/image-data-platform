...
The project SHALL migrate relevant image_data_platform documentation from `docs/old/` into the structured `docs/image_data_platform/` tree using the defined templates, and SHALL deprecate the old documents as non-canonical after migration. The project SHALL maintain a detailed mapping checklist to verify the completeness and accuracy of the migration.
...
#### Scenario: Verification of Migration Completeness
- **WHEN** verifying that all legacy documentation has been properly migrated
- **THEN** the project SHALL maintain a detailed mapping checklist (`docs/image_data_platform/OLD_DOCS_MAPPING_CHECKLIST.md`)
- **AND** the checklist MUST enumerate all files in `docs/old/` (excluding archive directories)
- **AND** each file MUST be mapped to its corresponding location in `docs/image_data_platform/` or marked as archived/historical reference
- **AND** the checklist MUST be updated when new mappings are established or verified
- **AND** the checklist MUST be referenced in the main documentation index (`docs/image_data_platform/00_IMAGE_DATA_PLATFORM_INDEX.md`).
