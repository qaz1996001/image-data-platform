...
- Store the matrix in this proposal as **the authoritative alignment snapshot** for Phase 1.
- Future OpenSpec changes that materially alter UR / SYS‑SR / FE‑SR / BE‑SR relationships should:
  - Update the same matrix (or move it into a dedicated OpenSpec spec file and reference it here).
...
- Adjust the example/placeholder table in `01_SYSTEM_PRD_SR_SD.md` Section 6 so that:
  - It either:
    - References this canonical matrix, or
    - Uses the same FE‑SR / BE‑SR IDs and wording as 02/03.
...
| UR‑005 | 單筆 AI 報告分析（Ollama、本地 LLM） | SYS‑PRD‑001 | SYS‑SR‑005 | FE‑PRD‑004 / FE‑SR‑040 | BE‑PRD‑004 / BE‑SR‑040 |
...
| UR‑009 | 高效 UI / 鍵盤快捷鍵 / 大列表效能 | SYS‑PRD‑004 | SYS‑SR‑010 + SYS‑SR‑NFR‑001, 004, 005 | FE‑PRD‑006 / FE‑SR‑080（並間接依賴 FE‑SR‑010, 011, 020 等） | BE‑PRD‑001, BE‑PRD‑002, BE‑PRD‑003 / BE‑SR‑NFR‑001 – 005（後端僅以效能／穩定性支撐） |
