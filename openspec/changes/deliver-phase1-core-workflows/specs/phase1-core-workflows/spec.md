...
- 給定使用者輸入關鍵字與多個篩選條件時，前端 **SHALL** 呼叫 `GET /api/v1/studies/search` 並傳遞 `page`, `page_size`, `sort`、`filters`（含 `exam_status`, `exam_source`, `exam_item`, `date_range`, `has_annotation`, `project_id` 等）欄位。
...
UR-007 **SHALL** 規範專案 CRUD、RBAC 暴露與 selection cart 批次操作，確保角色權限能在 UI 與 API 之間一致。

#### Scenario: 角色感知的專案列表
- `GET /api/v1/projects` 回傳的每筆資料 **SHALL** 含 `role` 欄位（owner/admin/editor/viewer），前端 **SHALL** 依此決定可用操作：只有 owner/admin 可以刪除專案，editor 可以新增/移除 study，viewer 僅能閱讀。
- Projects 頁面 **MUST** 在卡片/列表上顯示使用者角色，並在嘗試超權限操作時顯示 403 訊息。

#### Scenario: Shopping cart → Projects 批次指派
- 當 Study Search 選取多筆資料並點「加入專案」時，前端 **SHALL** 將選取的 `exam_id` 傳給 `POST /api/v1/projects/{project_id}/studies`，並 **SHALL** 遵守後端設定的 500 筆批次上限。
- 成功指派後，前端 **MUST** 清空 selection cart 並重新載入專案與搜尋結果；若後端回傳 `failed_items`（例如已存在或無權限），UI **SHALL** 列出詳細錯誤讓使用者處理。
